...
from langgraph_open_ttd_dr.schema import Queries
...
async def generate_sub_gaps_query(state: State):
    ...
    prompt = gap_analysis_prompt.format(query=query, research_plan=plan, draft=draft, last_all_q=last_all_q)
    messages = [{"role": "user", "content": prompt}]
...
builder.add_edge("draft_denoising", END)